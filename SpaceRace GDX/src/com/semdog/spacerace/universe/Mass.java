...
public abstract class Mass implements GoalObject, Disposable {
    ...
    public void debugRender(ShapeRenderer renderer) {
        ...
        if (getEccentricity() < 1) {
            ...
            /*float a = position.x;
            float b = position.y;
            float m = velocity.angle() * MathUtils.degreesToRadians;
            float r = environment.getSOI();

            float x1 = (float) ((-Math.sqrt(-a*a*m*m + 2*a*b*m - b*b + m*m*r*r + r*r) + a*m*m - b*m)/(m*m+1));
            float x2 = (float) (-(-Math.sqrt(-a*a*m*m + 2*a*b*m - b*b + m*m*r*r + r*r) + a*m*m - b*m)/(m*m+1));

            float y1 = m * (x1 - a) + b;
            float y2 = m * (x2 - a) + b;

            renderer.line(position.x, position.y, x2, y2);*/
            float fx = position.x + velocity.x / 4;
            float fy = position.y + velocity.y / 4;
            renderer.line(position.x, position.y, fx, fy);
            float a = velocity.angle() * MathUtils.degreesToRadians;
            float e = -2.2f;
            renderer.line(fx, fy, fx + 15 * MathUtils.cos(a + e), fy + 15 * MathUtils.sin(a + e));
            renderer.line(fx, fy, fx + 15 * -MathUtils.cos(a - e - MathUtils.PI), fy + 15 * -MathUtils.sin(a - e - MathUtils.PI));
            //renderer.line(fx, fy, fx - 15 * MathUtils.cos(a - 1 - MathUtils.PI / 2f), fy - 15 * MathUtils.sin(a - 1 - MathUtils.PI / 2f));
            //renderer.line(fx, fy, fx + 15 * MathUtils.cos(a - 1 - MathUtils.PI / 2f), fy + 15 * MathUtils.sin(a - 1 - MathUtils.PI / 2f));
        }
    }
